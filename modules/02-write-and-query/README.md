...
# Module 02 · Write & query time-series data

Bulk-load rows, bucket them into intervals, and join a hypertable with ordinary metadata.

**Run it:** `npm run lesson 02` · ~9 min
...
## 1. The dataset you are working with

Two tables: sensors is an ordinary PostgreSQL table of metadata, and readings is the hypertable of measurements. This split is the normal pattern - keep slow-changing metadata relational, and put the high-volume time-stamped facts in the hypertable.
...
> **Takeaway** Routing happens on write, so there is no partition maintenance job for you to run and no chance of inserting into the "wrong" partition.

## 3. Bulk loading: let the server generate the rows

Row-at-a-time inserts are dominated by round trips. The three fast options are COPY, multi-row INSERT, and - when the data is derived - INSERT ... SELECT, which never leaves the server. Here we generate a full day of one-minute readings for a new sensor in a single statement.
...
> **Note** For loading a file from a client, COPY is the fastest path: COPY readings FROM STDIN WITH (FORMAT csv).

## 4. How many rows did that add?
...
_Hint: Join readings to sensors, filter on time, and GROUP BY site._

<details><summary>Solution</summary>

```sql
SELECT s.site, avg(r.temperature) AS avg_temp FROM readings r JOIN sensors s USING (sensor_id) WHERE r.time >= now() - INTERVAL '24 hours' GROUP BY s.site ORDER BY s.site;
```
...
Next: npm run lesson 03  (continuous aggregates)
